...
contract SoulBound is ERC721A, ERC2981, AccessControl, Initializable{
    ...
    function mint(address _to, uint256 _tokenId) public onlyOwner {
        // Check if the token is already soul-bound
        require(!soulBound[_to]);

        // Mint the token and mark it as soul-bound
         _safeMint(_to, _tokenId);
        soulBound[_to] = true;
    }
    function burn(address _from, uint256 _tokenId) public {
        // Check if the token is soul-bound
        require(!soulBound[_from]);

        // Burn the token
        _burn(_tokenId);
    }
    function revokeAccess(address _tokenOwner, address _user) public {
        require(soulBound[_tokenOwner]);
        accessControl[_tokenOwner][_user] = false;
    }

    function grantAccess(address _tokenOwner, address _user) public {
        require(soulBound[_tokenOwner]);
        accessControl[_tokenOwner][_user] = true;
    }
    function checkAccess(address _tokenOwner, address _user) public view returns (bool){
        return accessControl[_tokenOwner][_user];
    }
